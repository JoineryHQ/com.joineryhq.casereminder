...
/**
 * CaseReminderJobRecipientError API Test Case
 * @group headless
 */
class api_v3_CaseReminderJobRecipientErrorTest extends \PHPUnit\Framework\TestCase implements HeadlessInterface, HookInterface, TransactionalInterface {
  ...
  private $caseReminderJobRecipientId;
  ...
  public function setUp(): void {
    $table = CRM_Core_DAO_AllCoreTables::getTableForEntityName('CaseReminderJobRecipientError');
    $this->assertTrue($table && CRM_Core_DAO::checkTableExists($table), 'There was a problem with extension installation. Table for ' . 'CaseReminderJobRecipientError' . ' not found.');
    ...
    $this->caseReminderJobRecipientId = $caseReminderJobRecipientCreate['id'];
  ...
  public function testCreateGetDelete(): void {
    // Create a job recipient error, populating all fields.
    $apiParams = [
      'job_recipient_id' => $this->caseReminderJobRecipientId,
      'error_message' => 'This is an error message',
    ];
    $caseReminderJobRecipientCreate = $this->callAPISuccess('CaseReminderJobRecipientError', 'create', $apiParams);
    ...
    $get = $this->callAPISuccess('CaseReminderJobRecipientError', 'get', []);
    ...
    $this->callAPISuccess('CaseReminderJobRecipientError', 'delete', [
      'id' => $created['id'],
    ]);
  }
  ...
  /**
   */
  public function testErrorMessageIsRequired(): void {
    // Create a job recipient error, populating all fields.
    $apiParams = [
      'job_recipient_id' => $this->caseReminderJobRecipientId,
    ];
    $caseReminderJobRecipientCreate = $this->callAPIFailure('CaseReminderJobRecipientError', 'create', $apiParams);
  }

  public function testErrorMessageIsTrimmed(): void {
    $overlongErrorMessage = str_pad('String with 300 characters', 300, '-');

    // Create a job recipient error, using an over-long error message
    $apiParams = [
      'job_recipient_id' => $this->caseReminderJobRecipientId,
      'error_message' => $overlongErrorMessage,
    ];
    ...
    try {
      ...
      $this->assertStringContainsString('Value for error_message is 300 characters  - This field has a maxlength of 255 characters.', $e->getMessage(), 'API fails with "max length" error message?');
    }

    // Create a job recipient error, using a trimmed over-long error message
    $apiParams = [
      'job_recipient_id' => $this->caseReminderJobRecipientId,
      'error_message' => substr($overlongErrorMessage, 0, 255),
    ];
